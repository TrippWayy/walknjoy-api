...
const accountCloud = async (req, res, next) => {
    ...
    try {
        if (req.user) {
            image = path.join(__dirname, `../uploads/avatars/${req.user.username}_pp`);
            result = await cloudinary.v2.uploader.upload(image, {public_id: `${req.user.username}_pp`})
        } else {
            image = path.join(__dirname, `../uploads/avatars/${req.body.username}_pp`);
